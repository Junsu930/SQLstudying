...
-- 문자열 5번째 문자부터 검색하여 첫 번째 B의 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL;

-- 문자열 5번째 문자부터 검색해서 두 번째 B의 위치 조회
...
SELECT SUBSTR('dillder of mdd', 4, 2) FROM DUAL;
...
SELECT EMP_ID, EMP_NAME 
FROM EMPLOYEE 
WHERE MOD(EMP_ID,2) <> 0; -- <> 같지 않다 (!=과 같음)

-- ROUND(숫자 | 컬럼명 [, 소수점 위치]) : 반올림
SELECT ROUND(123.456) FROM DUAL; -- 소수점 첫번째 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DUAL; -- 소수점 두번째 자리에서 반올림 == 소수점 첫번째 자리 까지 나옴

SELECT ROUND(123.456, 0) FROM DUAL; -- 기본값

SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0번째 자리에서 반올림해서 소수점 -1의 자리까지 표현
...
-- TRUNC(숫자 | 컬럼명 [, 위치]) : 특정 위치 아래를 버럼(절삭)

SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래를 버림 

SELECT TRUNC(123.456, 1) FROM DUAL; -- 소수점 첫쨰자리 아래 버림
...
SELECT EMP_NAME 사원명, HIRE_DATE 입사일, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) 근무개월, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/ 12) || '년차' 근무연차 
FROM EMPLOYEE;
...
-- EXTRACT(MONTH FROM 날자) : 월만 추출
-- EXTRACT(DAY FROM 날자) : 일만 추출
...
-- 년, 월, 일 날짜를 나타내는 패턴으로 인식이 안돼서 오류 발생
...
SELECT EMP_NAME, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6), 'RRMMDD'), 'YYYY"년" MM"월" DD"일"') 생년월일 -- 6 대신에 INSTR(EMP_NO, '-')-1으로 해주는 게 더 정확
FROM EMPLOYEE;
...
SELECT EMP_NAME, EMP_NO, DECODE( SUBSTR(EMP_NO,8, 1), '1', '남성', '2', '여성') 성별 FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 급여가 500만원 이상이면 '대' 300만 이상이면 500만 미만이면 '중' 300만 미만이면 '소'
...
-- EMPLOYEE 테이블에서 성별이 남성이 사원의 수 조회
SELECT COUNT(*) 
FROM EMPLOYEE
