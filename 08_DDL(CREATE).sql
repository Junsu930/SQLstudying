...
INSERT INTO USER_USED_CHECK VALUES (1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-1234', 'hing@naver.com');

SELECT * FROM USER_USED_CHECK;

INSERT INTO USER_USED_CHECK VALUES (1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-1234', 'hing@naver.com');

-- ORA-02290: 체크 제약조건(KH.GENDER_CHECK)이 위배되었습니다
-- CHECK 제약조건으로 GENDER 컬럼을 설정했기 때문에, 남 또는 여만 기록 가능한데
-- 남자라는 조건 이외의 값이 들어와서 에러 발생
...
COMMENT ON COLUMN USER_TEST.PHONE IS '회원핸드폰번호';
COMMENT ON COLUMN USER_TEST.ADDRESS IS '회원주소';
COMMENT ON COLUMN USER_TEST.STATUS IS '회원상태';


----------------------------------------------------------------------------------------------------------
-- 6. SUBQUERY 이용한 테이블 생성법
-- 컬럼명, 데이터 타입, 값이 복사되고, 제약조건의 NOT NULL만 복사됨
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_DEPT FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT ON DELETE SET NULL; 


-- EMPLOYEE테이블의 JOB_CODE 외래키 제약조건 추가
-- 참조 테이블은 JOB, 참조 컬럼은 JOB의 기본키
ALTER TABLE EMPLOYEE ADD CONSTRAINT EMP_JOB_CODE FOREIGN KEY(JOB_CODE) REFERENCES JOB ON DELETE SET NULL;


-- EMPLOYEE테이블의 SAL_LEVEL 외래키 제약조건 추가
-- 참조 테이블은 SAL_GRADE, 참조 컬럼은 SAL_GRADE의 기본키
ALTER TABLE EMPLOYEE ADD CONSTRAINT EMP_SAL_GRADE FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE ON DELETE SET NULL; 

-- DEPARTMENT테이블의 LOCATION_ID에 외래키 제약조건 추가
-- 참조 테이블은 LOCATION, 참조 컬럼은 LOCATION의 기본키
ALTER TABLE DEPARTMENT ADD CONSTRAINT DEPT_LOCATION FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION ON DELETE SET NULL;



-- LOCATION테이블의 NATIONAL_CODE에 외래키 제약조건 추가
-- 참조 테이블은 NATIONAL, 참조 컬럼은 NATIONAL의 기본키
ALTER TABLE LOCATION ADD CONSTRAINT LOC_NATIONAL FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL ON DELETE SET NULL;
