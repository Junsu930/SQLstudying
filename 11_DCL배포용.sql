...
-- 2. 새 연결(접속 방법) 추가
--> ORA-01045 : 사용자 kjs_sample은 CREATE SESSION 권한 -> 접속 권한 이 없다

-- 3. 접속 권한 부여 
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO kjs_sample;
...
-- 6. (SYS) 테이블 생성 권한 + TALBESPACE 할당
GRANT CREATE TABLE TO kjs_sample;

ALTER USER kjs_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 할당
...
-- kh / kjs_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) kjs_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;

GRANT SELECT ON EMPLOYEE TO kjs_sample;

-- 3. (sample) 다시 kh.EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE;

-- SELECT * FROM kh.DEPARTMENT; --> DEPARTMENT 테이블 조회 권한은 준 적 없으므로 조회 불가
...
REVOKE SELECT ON EMPLOYEE FROM KJS_SAMPLE;
