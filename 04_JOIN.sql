...
----------------------------------------------------------------------------------------------------------------
카테시안(카티션) 곱		              			 교차 조인(CROSS JOIN) - 잘못된 데이터 검증 위해서 사용, 거의 사용하지 않는다
CARTESIAN PRODUCT
...
SELECT EMP_NAME, DEPT_TITLE 
FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 23행 (하동운, 이오리 포함)
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE 
FROM EMPLOYEE RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 24행 (모든 부서 포함)
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT;  -- 207 행 == (EMPLOYEE)23행 * (DEPARTMENT)9행 = 207행
...
WHERE E.DEPT_CODE = D.DEPT_ID -- EMPLOYEE + DEPARTMENT 조인
AND D.LOCATION_ID = L.LOCAL_CODE; -- (EMPLOYEE + DEPARTMENT) + LOCATION 조인
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
LEFT JOIN JOB USING (JOB_CODE)
WHERE DEPT_TITLE LIKE '해외영업1부'
OR DEPT_TITLE LIKE '해외영업2부'
...
SELECT EMP_NAME, BONUS, NVL(DEPT_TITLE, '부서없음'), NVL(LOCAL_NAME, '지역미정')
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT EMP_NAME, JOB_NAME, SALARY, 12*(SALARY+SALARY*NVL(BONUS, 0)) 연봉
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- SELF JOIN 사용
SELECT e1.EMP_NAME 사원명 , e1.DEPT_CODE 부서코드 , e2.EMP_NAME 동료
FROM EMPLOYEE e1
