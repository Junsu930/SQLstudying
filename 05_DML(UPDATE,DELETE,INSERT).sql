...
-- 선택 안된 컬럼은 값이 NULL이 들어감 (DEFAULT 존재 시 DAFAULT 값으로 삽입됨)

INSERT INTO EMPLOYEE2(EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
VALUES ('987','김팔팔', '930331-1234567', 'palpal@naver.com','01099992222', 'D1', 'J7', 'S3', 4500000);
...
-- UPDATE 확인
SELECT * FROM DEPARTMENT2 
WHERE DEPT_ID LIKE 'D9'; -- 'D9' 전략기획팀으로 수정됨을 확인!
...
AS SELECT * FROM EMPLOYEE -- EMPLOYEE를 본떠 만들었으나 JOB_CODE 가 J4인 값만 본떠 만듦
WHERE JOB_CODE = 'J4';
...
SELECT * FROM EMPLOYEE2 WHERE EMP_NAME = '김팔팔';

DELETE FROM EMPLOYEE2 WHERE EMP_NAME = '김팔팔'; --> 조회 결과 없음
...
SELECT EMP_NAME, EMAIL  FROM EMPLOYEE WHERE JOB_CODE = 'J5'
MINUS
