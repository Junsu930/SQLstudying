...
SELECT DEPT_CODE, SUM(SALARY) FROM EMPLOYEE GROUP BY DEPT_CODE; --> DEPT_CODE가 같은 행끼리 하나의 그룹이 됨

-- EMPLOYEE 테이블에서
-- 직급 코드가 같은 사람의 직급 코드, 급여 평균, 인원 수를 
-- 직급 코드 오름차순으로 조회

SELECT JOB_CODE, COUNT(*), ROUND(AVG(SALARY))  
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
SELECT DECODE(SUBSTR(EMP_NO,8,1), '1','남','2','여') 성별 ,COUNT(*) "인원 수", SUM(SALARY) 급여합 
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO,8,1), '1','남','2','여') -- 별칭 사용이 X (SELECT절 해석 X)
ORDER BY "인원 수" ; -- 별칭 사용이 0 (SELECT 절 해석 완료)
...
-- EMPLOYEE 테이블에서 직급별 2000년도 이후 입사자들의 급여 합을 조회 직급코드 오름차순

SELECT JOB_CODE 직급코드 , SUM(SALARY) "급여 합"   
FROM EMPLOYEE 
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000 -- HIRE_DATE >= TO_DATE('2000-01-01'),  TO_NUMBER(SUBSTR(TO_CHAR(HIRE_DATE),1,4)) >= 2000
GROUP BY JOB_CODE
ORDER BY 직급코드;
...
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE로 묶은 그룹 중 급여 평균이 300만 이상인 그룹만 남음
ORDER BY DEPT_CODE;
