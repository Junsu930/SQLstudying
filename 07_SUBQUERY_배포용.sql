...
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME ='과장');
-- > ANY : 과장 급여 3개 중, 최소금액인 220만보다 더 큰 SALARY
-- < ANY : 과장 급여 3개 중, 최대금액인 376만보다 더 작은 SALARY 
...
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '과장'
AND SALARY > ALL (SELECT SALARY /*MAX(SALARY)로 구해도 된다*/
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '차장')
...
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, HIRE_DATE 
FROM EMPLOYEE 
WHERE DEPT_CODE = (SELECT DEPT_CODE, JOB_CODE FROM EMPLOYEE 
WHERE SUBSTR( EMP_NO, 8, 1 ) = 2  -- 여직원
AND ENT_YN = 'Y')
...
SELECT EMP_NAME, JOB_CODE, SALARY, (SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE MAIN.JOB_CODE = SUB.JOB_CODE ) FROM EMPLOYEE MAIN;
